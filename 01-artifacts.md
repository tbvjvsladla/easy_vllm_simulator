...
**triplet**
- `output/single/configs/fp8-dev-262k.yaml`
- `output/single/configs/fp8-dev-262k.sh`
- `output/single/envs/.env.fp8-dev-262k`
...
- **triplet — 적용.** 서빙에 원리적으로 필수다. 이 형상의 핵심은 세 줄이다 —
  `kv-cache-memory-bytes`(4 GiB 절대클램프) · `max-model-len: 262144`(없음(원본 상한)) · `enforce-eager`.
  PLE 배치는 상주(오프로드 노브 없음) 로 선언된다. `max-num-seqs: 4` 도 이 트리플렛의 통제변인이다 —
