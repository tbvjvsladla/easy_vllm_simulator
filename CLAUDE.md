...
> 근거: Ouroboros 인터뷰 `interview_20260606_205859` → Seed `seed_26269d6807f5`.

## 정체성

- **업스트림(진실의 원천)**: vLLM — https://github.com/vllm-project/vllm
  - "버전" = GitHub Release 태그. **pre-release도 추적 대상**.
- **관리 대상**: 커스텀 Docker 컨테이너 = NGC PyTorch 베이스 + vLLM pre-built wheel + 주변 의존성.
...
- **브랜치**: `main` = 단일노드 전용 · `multi-node` = 분산 전용. 두 브랜치는 버전 핀이 독립.
...
- **2-브랜치 배포**: `main`(단일) · `multi-node`(분산) 모두 배포 대상. 공유 빌딩블럭은 `scripts/sync_branches.sh`로 동일하게 유지.

## build / 검증 커맨드

- 이미지 빌드(디버그): `docker compose --profile debug build`
- 서빙: `docker compose --env-file envs/.env.<config> --profile serve up`
...
- **smoke-before-commit**: 로컬 빌드+스모크 통과 코드만 last-good 커밋으로 남긴다.
  (origin은 사용자 환경 값(`manifest.origin_url`)에서 설정 — 없으면 로컬 전용. 멀티노드 서브 전파는 rsync `sync_to_sub.sh`.)
...
- **모델을 자체적으로 다운로드하지 않는다.** NAS에 없으면 즉시 **중단·보고**.
...
- 실패 시: `git reset --hard <last-good-commit>`로 복귀(잔여 로컬 상태 0). `main`·`multi-node` 독립 롤백.
...
- **커스텀**: `/upstream-version-watch` — 업스트림 추적 → 변경 독해 → 레이어 bump 제안 (Step 2에서 설계).
- **외부**: Docker 작성/문법검사 보조 — 후보 `netresearch/docker-development-skill` (설치정책 거쳐 도입).
- **MCP**: 현재 없음. (멀티노드 서브노드 직접 SSH 제어 = 구현됨: sync_to_sub.sh rsync 전달 +
  서브 빌드워커 CC `ssh sub bash -lc "claude -p"`. SKILL.md §4.5 / workflow.md S2.5·S3.)
- 참고: 기존 `configs/check_reqs.py`(의존성 차이 분석 스크립트)를 Step 2의 결정론적 resolve 기반으로 재활용 예정.
...
- 부트스트랩 근거: `seed/bundle_pkg/`(GUIDE·question-catalog·REFERENCES), Seed: `seed/seed_26269d6807f5.yaml`.
