...
- 메인이 서브에 하는 행위는 **저작·스캔·정비** 3범주다(정본 표는 workflow.md). **저작**(콘텐츠 생성·
  수정)은 템플릿→렌더→배달 파이프라인으로만, **스캔**(디스크 내용 읽기)은 금지이며 상향 회수는 문서
  기반이다(PII 격리). **정비** — 정보량 0인 상태 결손(빈 디렉터리·mode 오차)의 배달 전제 복구 — 는
  허용하며 로그를 남긴다: 만들지도 읽지도 않으므로 앞의 두 금지에 걸리지 않는다. git 경유 이력 관측은
  관측-장치 원칙에 따라 허용된다.
- 평면 B에 게이트를 놓을 때는 그 처방을 **누가 실행하는가**를 먼저 적는다 — 메인이 아니면 교착이다
  (`SUB_SYNC_DIRTY_AUTOSAVE`·정비 범주가 같은 클래스의 두 실증).
