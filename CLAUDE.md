...
- 레이어 커플링: 대상 vLLM 버전이 선언한 torch 버전과 NGC 컨테이너 베이스의 torch 빌드버전이 접두어
  일치해야 한다. 빌드트랙(prebuilt wheel 대 소스빌드) 판정과 세대 경계·보강 원칙은 스킬
  upstream-version-watch가 소유하며, 최종 중재는 항상 스모크다(상세·예시는 references.md).
...
- serve-time 변수 해소는 환경 주입이 manifest보다, manifest가 리터럴 기본값보다 우선한다(절차는
  workflow.md).
- 모델 구동 아티팩트(트리플렛·런타임 패치·빌드-바깥 패치)는 평면·소유·전달이 슬롯별로 다르다 — 발견이
  곧 소유는 아니다(상세는 workflow.md).
