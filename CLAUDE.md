...
- **KV 절대클램프 따름정리 (이식성)**: 최종 recipe 의 KV 는 **측정된 GPU당 `kv-cache-memory-bytes`(절대값)** 로 제어한다(gmu-derived KV=호스트 VRAM 차이로 비이식·OOM) — **단 `gpu-memory-utilization` 도 함께 emit**: gmu=startup free-memory 게이트+총-cap(통합메모리는 **gmu ≤ 0.90 명시 필수**), clamp=KV 사이징·이식성. 상세(cache.py 근거·free/total 수치) = 스킬 `vllm-recipe-explorer` §5 · `plan_2026062711_1` Part 2.
...
- **적대적 성능 검증 따름정리 (기능≠성능 · self-preference 차단)**: **기능 스모크만으로 done 선언 금지 — 성능은 `adversarial-benchmark` 적대 루브릭 게이트로 별도 검증**한다(3중 방어막 (a)루프라인→(b)외부 E→(c)사용자 · spec-aware like-with-like(R_fp↔R_token 혼동 ✗) · PASS/REFUTE=결정론 `verdict_rule.py`, LLM 다수결 ✗). recipe 와 loop-until-done(기각→재탐색·cap 한정), (b) E arm 은 메인전용(서브=루프라인-only+증상 상향). 상세(공식·측정 정본·실패축) = 스킬 `adversarial-benchmark` · `plan_2026063014_1`.
...
- **모델 획득 모드 따름정리 3종 (managed|ephemeral|custom · *모델 weights 획득* 한정)**: manifest `model_source` 가 다운로드 정책을 정한다 — **managed**(관리 NAS read-only·무단 HF ✗) · **ephemeral**(컨테이너 임시·down→삭제 — **다수 기본**) · **custom**(사용자 지정 경로 영속·볼륨마운트). 게이트: 모델 부재 시 **per-event 사용자 승인**으로만 모드 기본위치에 다운로드('사전승인'=모드 허가일 뿐 무인 포괄허가 ✗). **단 *서빙전략 수립*의 외부 교차검증(HF 카드·vLLM GitHub)은 획득모드와 무관하게 항상 의무**(폐쇄망=모델획득 한정, 외부접속 전반 차단 아님). 절차 정본 = workflow.md §모델/안전 · `plan_2026063018_1`.
...
- **escalation 역루프 따름정리 (recipe→upstream · 발견≠소유)**: 새 모델이 현 vLLM 으로 *구조적* 불가일 때 recipe 가 **오프라인 증상 ∧ 외부 확증 둘 다**(단일 신호 ✗)로 발견 → **사용자 명시 승인** → upstream 이 버전핀 소유·처방(**3출구**: (i) 공식 bump / (ii) 포크핀 변종 트랙 / (iii) 음성정직 — references.md §5 최소범위 레시피 수행 후에만) → rebuild 후 recipe 재개. 게이트: 드문 예외 경로(대다수 모델은 그냥 뜸·매 요청 외부리서치 ✗) · 무승인 자동 escalate ✗ · 순환=reconciliation_cap · 최종 중재=스모크 · 서브(에어갭)=증상 상향만. 절차 정본 = workflow.md §escalation · 스킬 recipe §5.5 ↔ upstream §3.6 · `plan_2026063009_2`.
