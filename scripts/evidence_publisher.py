...
Implements the 7-step hybrid-publisher contract from plan_26072506 §4.4:
    ...
    5. delegate schema/path/identity/link/PII/verdict verification to scripts/completion_gate.py
...
stdlib only, no third-party dependencies -- reuses scripts/completion_gate.py and
scripts/doc_naming.py rather than reimplementing their logic (path-safety resolvers, the
...
# entire schema/identity/link/PII/verdict verification to scripts/completion_gate.py (never a
...
def _self_test() -> None:
    ...
    invokes the real scripts/completion_gate.py -- this is not a mocked check."""
    import contextlib
    import io
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 0 and out and out["ok"], out
        assert sorted(out["required_evidence"]) == ["commit", "verification"], out
        ...
        assert out is not None and out.get("state") == "evidence-complete", out
        assert out.get("eligible_for_promotion") is False, out  # minor_patch is promotion-capped
