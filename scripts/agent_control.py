#!/usr/bin/env python3
"""scripts/agent_control.py -- provider-neutral agent-control orchestrator (Phase 6, plan_26072506).

Reads a request JSON conforming to `.claude/schemas/agent-control-request.schema.json`, dispatches
it to the requested provider adapter under `scripts/providers/` (Phase 6 scope: `claude_code` only
-- see scripts/providers/claude_code.py), and prints a stable
...
lives entirely in scripts/providers/claude_code.py) -- see
tests/harness/test_skill_contracts.py::TestClaudeCommandsIsolatedToAdapter.
...
# $ref/allOf/if-then-else. Modeled on scripts/completion_gate.py:validate_against_schema.
# =============================================================================
...
def _load_provider(provider_name: str):
    """Dynamically loads scripts/providers/<provider_name>.py -- pure routing, no provider-specific
