...
def verify() -> dict:
    ...
    checks.append({"name": "root_tests_contract", "ok": tests_root.is_dir()
                   and (tests_root / "test_a2a_identity.py").is_file()
                   and (tests_root / "test_resolve_render_input.sh").is_file()})
    ...
    for rel in TRUST_FILES:
        ...
        try:
            ...
            stale = sorted({s for s in _walk_strings(doc) if s.startswith("scripts/")})
        ...
        checks += [
            ...
            {"name": "sub_transactional_source_uses_git_index",
             "ok": ("checkout-index -z --stdin" in sub_text
                    ...
                    and 'required render input missing' in sub_text
                    and 'render_input="sub_manifest.yaml"' in sub_text
                    and 'optional render input absent' in sub_text
