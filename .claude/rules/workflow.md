...
#### 메인이 서브에 하는 행위의 3범주 (2026-08-14 신설 · `plan_26081409` C)

*"서브 디스크를 무단 스캔하거나 직접 교정하지 않는다"* 는 한 문장이 **두 관심사**(격리·저작)를 묶고
있었고, 그 결과 **어느 쪽도 아닌 행위**까지 함께 막아 교착을 만들었다. 세 범주로 가른다:

| 범주 | 무엇 | 허용 | 근거 |
|---|---|---|---|
| **저작** | 서브 작업환경·헌법 콘텐츠를 **만들거나 고침** | 템플릿→렌더→배달 **only** | 재현성(직접 교정은 드리프트) |
| **스캔** | 서브 디스크 내용을 **읽음** | **금지**(상향 회수는 문서기반) | PII 격리 |
| **정비**(신설) | **정보량 0**인 상태 결손 복구(빈 디렉터리·mode 오차) | **허용** · 로그 필수 | 배달 전제 복원 |

정비가 저작도 스캔도 아닌 이유: **만들지 않고**(제거·정정만) **읽지 않는다**(내용이 없다).
정보량 0 이라는 조건이 범주의 경계다 — 바이트를 가진 것을 지우는 것은 정비가 아니라 삭제이며,
`ALLOW_DELETE` 게이트가 그대로 관할한다.

**왜 필요했나**(2026-08-14 실증): 롤백 스냅샷이 `find -type f -o -type l` 로 파일·심링크만 담는데,
**빈 디렉터리는 파일 경로로 함의되지 않는 유일한 디렉터리**라 복원 불가다. 그래서 검증기가 배달을
거부했고 — 거부 뒤 복구 경로가 없어 **사람의 ssh `rmdir`** 이 필요했다(2026-08-01 엔 매번 사람 sudo).
메인은 그 권한을 이미 갖고 있었으나 **파이프라인이 쓸 줄 몰랐다.**

> **일반 명제**: 게이트의 처방이 가리키는 주체가 아키텍처에 없으면, 그 게이트는 안전장치가 아니라 교착이다.
> `SUB_SYNC_DIRTY_AUTOSAVE`(2026-08-13)와 이 정비 범주(2026-08-14)는 **같은 클래스의 두 인스턴스**다.
> 평면 B 에 새 게이트를 놓을 때는 그 처방을 **누가 실행하는가**를 먼저 적어라 — 메인이 아니면 교착이다.

⚠ 정비의 범위는 **전송 범위와 일치**해야 한다. `BAND2_EXCLUDED_TOP` 아래는 rsync 가 건드리지 않으므로
정비도 건드리지 않는다 — 최상위 디렉터리 자신뿐 아니라 **하위 전부**다(2026-08-14 초판이 이걸 어겨
`cache/vllm/…` 를 정비했다. 대조실험이 잡았다).
