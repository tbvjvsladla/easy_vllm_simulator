...
- S4 뒤 공유 빌딩블럭 동기화는 사람 질의로만 `scripts/sync_branches.sh`; hint 후보는 `policy:HINT_TAG_ACTIVATION_GATE`와 `scripts/hint_tag.py`의 match→create→judgment→finalize→verify 순서만 허용한다.
