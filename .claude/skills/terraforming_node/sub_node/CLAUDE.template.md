...
- 한 Task = 하나 `context_id`. 진행상태는 `campaigns/<camp-id>/relay/<context_id>.json` 에 산다(**파일=세션**).
  활성 캠페인을 모르면 예약 id `_bootstrap` 을 쓴다 — **루트에 쓰지 않는다**(옛 `tasks/` 는 2026-09-06 폐기).
  이전 턴·피드백을 읽고, 네 턴을 덧쓴다.
