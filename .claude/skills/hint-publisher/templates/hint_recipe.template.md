<!-- ⚠ 절 헤딩(`## 1.` ~ `## 7.`)을 **지우지 마라** — `hint_tag seal` 의 린터 L1 이 fail-closed 로
     막는다. 2026-08-20 실측: 이 템플릿이 처음부터 있었는데도 발행된 49/49 태그에 헤딩이 0개였고
     밀도가 12배 벌어졌다. 템플릿만으로는 아무것도 보장되지 않는다(plan_26082009 §1). -->
