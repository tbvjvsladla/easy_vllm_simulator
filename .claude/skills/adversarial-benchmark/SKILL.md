...
- `scripts/resolve_topology.py` — **벤치 6종(broad_search·judge_bench·lite_bench·run_bench·sweep_bench·max_envelope)의 `--topology` 미지정 해소기**(2026-09-14 · ⑧ 분석 발견 T8 · 헌법 "토폴로지는 manifest 에서 읽고 브랜치로 추론하지 않는다"). 종전 관용구는 `git rev-parse --abbrev-ref HEAD` 브랜치 이름으로 골랐고 unknown 은 조용히 single 이었다. 판정하지 않고 두 소유자를 부른다: 서명된 `Agent_Card.json` 이 있는 노드(메인이 프로비저닝한 서브) = `agent_card_contract.py prove-identity` 의 topology ∧ 자기 manifest `topology` 일치 · 메인 = `topology_parity.py evaluate`(4자일치 PASS ∧ 그 통로 manifest 다리 ok). 종료 0=해소(stdout 한 줄 · stderr 출처) · 3=manifest 한 장도 없음·통로 manifest 부재(미테라포밍 — run_bench·lite_bench 는 exit 4 로 옮겨 smoke_clone A8 계약 보존, 나머지는 exit 2) · 5=해소 불가(술어 RED·카드 검증 실패·카드↔manifest 불일치·판정자 부재 — run_bench·lite_bench exit 4 · 나머지 exit 2). `--topology` 명시는 해소기를 부르지 않는다. broad_search 는 쓰는 서브커맨드(init·cell)에서만 · cell·max_envelope 은 `--confirm-risk` 게이트 **뒤**에서 부른다(인자 평면 가드 보존). **`--self-test`**(R1~R7 카드 경로 · P1~P5 실제 4자일치 술어 · S0~S11 6종 바이트 사본 실행과 음성대조 — verify_distribution 이 매번 부른다).
