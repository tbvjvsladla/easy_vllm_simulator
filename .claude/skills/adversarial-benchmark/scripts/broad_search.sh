...
MSG
  ...
  if [ "$SERVE_RC" = "0" ]; then
    set +e
    bash "$SDIR/sweep_bench.sh" "$CONFIG" --topology "$TOPO" --tool guidellm \
         --bench-budget-mib "$BENCH_BUDGET"
    MEASURE_RC=$?
...
cell = {
    ...
    "coordinates": {k: meta.get(k) for k in
                    ("quantization", "max_model_len", "kv_cache_dtype", "tp",
                     "image_tag", "moe_backend", "gpu_memory_utilization")},
    ...
    "measurement": {"bench_tool": meta.get("bench_tool"),
                    ...
                    "vllm": meta.get("vllm"), "gpu_model": meta.get("gpu_model")},
