...
## 종료 시퀀스 — 커밋 → 브랜치 동기화 → 서브 전파 (2026-09-12 신설 · policy:BRANCH_CONSTITUTION_LAYERING)

> 사용자가 "브랜치 동기화" 또는 작업 종료를 지시하면 이 절차가 돈다. 실행자는
> `scripts/closing_sequence.sh` 이고, 이 절은 그 스크립트가 **하지 않는 두 걸음**을 소유한다.

| 단계 | 누가 | 무엇 |
|---|---|---|
| ⓪ 전제 | 스크립트 | 4자일치 PASS · 잔존 워크트리 0 · 원장 동기 지점 조회 |
| ① 열거 | 스크립트 | 동기 지점 이후 공통층 변경 → **분류표 빈칸** JSON |
| **② 분류** | **Agent** | 파일마다 `common_promote` / `branch_only` 판정 + 근거 |
| **③ 승인** | **사람** | 분류표를 보고 1회 승인. **이 시퀀스의 유일한 사람 게이트** |
| ④~⑦ 집행 | 스크립트 | 원장 append·커밋 → 워크트리 동기화 → 서브 오버레이 → push |

### ② 분류 — Agent 가 판정한다

작업 중의 헌법 갱신은 **기본적으로 공통층에 들어간다**(그래야 잊히지 않는다). 종료 시점에 그
갱신분을 다시 읽고, 한쪽 토폴로지에서만 참인 것을 특화층으로 내린다. 판정 질문은 하나다:

> **이 문장이 반대 토폴로지 체크아웃에서 읽힐 가치가 있는가.**

- 있다 → `common_promote`. 비교·대조("왜 두 토폴로지가 다른가")는 **양쪽이 읽어야 하므로 공통**이다.
- 없다 → `branch_only`. 그 내용을 `*.topology.md` 로 옮기고 공통층에서 걷어낸 **뒤** ①을 다시 돌린다
  — 분류표는 최종 트리를 반영해야 한다.
- 계약기가 파생하는 값(서브 정체·활성 스킬·배달 평면)은 **어느 층에도 재기재하지 않는다**. 특화층은
  "왜"와 참조만 든다.

모든 행은 `provenance: agent-judged` 를 단다. 근거가 빈 행은 원장이 거부한다 — 근거 없는 판정은
사람이 검토할 수 없고, 검토할 수 없는 것을 승인하면 게이트가 이름만 남는다.
...
오분류는 두 번 걸린다. ③에서 사람이 근거를 보고 거르고, 통과해도 ⑤의 반대 브랜치 커밋에서
**tripwire ⑧ 의 공통층 어휘 스캔**이 RED 를 낸다. 그래서 사람이 한 번 잘못 승인해도 두 브랜치가
조용히 같아지지 않는다.

### 멈추는 자리

RED 만 멈춘다 — 채우지 않은 분류표(자리표시자 · execute 가 **어떤 부작용보다 먼저** 거부) · 4자일치 불일치 ·
잔존 워크트리 · 미커밋(자동 보관 ✗) · 공통층 갈라짐(cherry-pick 선행) · 공통층 갈라짐 **판정 불가**
(`COMMON_LAYER_UNJUDGED` — 판정하지 못한 덮어쓰기를 통과로 두지 않는다) · 같은 경로의 report 충돌.
나머지 결손(카탈로그 재파생 실패 · 서브 전파 실패)은 **기재하고 진행한다** — 단 원인은 말한다(원격 전용
hint 태그의 로컬 오브젝트 부재면 그 원격의 fetch 명령을 안내하고 자동 fetch 는 하지 않는다). 결정론이 과하면
시퀀스가 그 자리에서 영원히 선다.

### 갈라짐 판정은 무엇을 읽나 (2026-09-14 · plan_26091407 §9 ⑧-pre D2)

- **원장은 복제하지 않고 ref 에서 읽는다.** 원장은 동기화 제외라 대상 워크트리에는 없을 수 있다(single-node 는
  원장을 추적하지 않는다 · multi-node 는 자기 원장을 든다). ⑤ 의 판정은 출발 브랜치 ref 의 원장을 **읽기만** 해
  워크트리 원장과 entry_id 합집합으로 본다(`layer_ledger --ledger-ref`) — 대상에 사본을 두면 두 번째 자리다.
- **동기 지점 = 실제로 내려앉은 공통층 트리.** 원장 항목을 새것부터 거꾸로 훑어, 출발 계보의 커밋과 공통층
  트리가 같은 대상 커밋을 찾는다. 같은 분류표로 재실행한 회차의 착지도, ④ 가 막 적은 이번 항목(아직 미착지)도
  이렇게 해소된다. 과거 항목은 고치지 않고 차이는 `departure_drift` 로 표시한다. 후보가 여럿이면 **계보상 가장 새
  출발 커밋**에 맞는 것이 착지다 — 옛 출발 트리로 되돌린 대상 커밋은 착지가 아니라 대상 쪽 되돌림이다.
- **갈라짐 = 대상이 바꿨고 ∧ 덮으면 사라지고 ∧ 상대가 받지 않았다.** 동기 지점 이후 대상이 바꾼 파일 중 상대와
  바이트가 같거나, 대상의 바이트가 **동기 지점 이후** 상대 계보에 들어온 것은 흡수(`absorbed`)로 뺀다. 그래서
  처방(대상 전용 변경을 출발 브랜치로 cherry-pick)을 따르면 다시 돌린 ⑤ 가 풀린다 — 풀리지 않는 안내는 교착이다.
  동기 지점 이전 바이트로의 되돌림은 흡수가 아니다.
- **동기 지점이 없으면 blob 조상으로 판정한다** — 대상 바이트가 출발 이력에 한 번도 없었거나(삭제면: 출발이 지운
  적 없음), 대상이 출발 이력의 더 새 판본을 든 뒤 옛 판본(또는 삭제)으로 되돌렸으면 갈라짐이다. "언제부터" 를
  모르는 **약한 판정**이라 clean 줄에 그렇게 적는다. 판정 근거는 `basis` 로 드러난다.
- **원장 무결성이 깨졌으면 판정하지 않는다.** 같은 entry_id 다른 내용(`ledger-conflict`)·못 읽는 원장·풀리지 않는
  ref(`ledger-unreadable`)는 `unjudged`(basis=`ledger-integrity`) → `COMMON_LAYER_UNJUDGED` 로 멈춘다. 약한 판정으로
  접지 않는다. 출발 브랜치에 판정기 자체가 없을 때(도입 이전 브랜치)도 같은 코드로 멈춘다.
- **판정기도 정본 판본이어야 한다.** ⑤ 는 `sync_branches.sh` 와 함께 `layer_ledger.py` 를 워크트리로 먼저
  당긴다 — 대상의 옛 판정기로 판정하지 않는다(`LEDGER_JUDGE_STALE`).
- **서브 전파 ⑥ 의 통로는 체크아웃이 정한다.** `sync_to_sub --branch` 는 이 체크아웃의 4자일치
  (`topology_parity --expect`)와 같아야 하고 어긋나면 인가 전에 exit 12 다(특화층 오배달 · 옛 빌드킷 다운그레이드 차단).
  안내는 원인별이다 — `--branch` 값만 틀렸으면 이 체크아웃의 통로와 반대 체크아웃 두 선택지를, 체크아웃 자체가
  4자일치 RED(manifest·활성 캠페인·특화헌법)면 그 선언을 고치라고 말한다. 통과 줄은 판정 다리 수를 밝힌다.
