...
#
# 사용:  bash .claude/skills/upstream-version-watch/scripts/smoke_clone.sh
set -uo pipefail
...
a4() {
    ...
    selfname=".claude/skills/upstream-version-watch/scripts/smoke_clone.sh"   # 자기 자신은 PII 패턴 정의를 담으므로 제외.
